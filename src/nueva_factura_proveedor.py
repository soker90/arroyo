...
class NuevaFacturaProveedor(QWidget):
    def __init__(self, mainWindows, id):
        ...
        bd = Bbdd()
        self.nombre = bd.select("proveedor", None, "id=" + str(self.id), "name")[0][0]
        bd.close()
        self.mainWindows.setWindowTitle("Nueva Factura para " + self.nombre + " | Arroyo v" + mainWindows.version)

    def close(self):
            self.mainWindows.setCentralWidget(FacturasProveedor(self.mainWindows, self.id))
    ...
    def accept(self):
        ...
        columns = ["proveedor", "n_orden", "fecha", "importe"]

        bbdd = Bbdd()
        bbdd.insert(columns, data, "factura_proveedor")
        bbdd.close()
