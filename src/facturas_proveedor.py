...
class FacturasProveedor(QWidget):
	def __init__(self, mainWindows, id):
		...
		bd = Bbdd()
		nombre = bd.select("proveedor", None, "id="+self.id, "name")[0][0]
	...
	def initTree(self):
		bd = Bbdd()
		data = bd.select("factura_proveedor", "n_orden", "proveedor=" + str(self.id))
	...
	def editItem(self):
		self.mainWindows.editarFacturaP(self.id, self.itemSelected)

	def nuevaFactura(self):
		self.mainWindows.nuevaFacturaP(self.id)