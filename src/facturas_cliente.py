...
class FacturasCliente(QWidget):
	def __init__(self, mainWindows, id):
		...
		bd = Bbdd()
		nombre = bd.select("cliente", None, "id="+self.id, "name")[0][0]
	...
	def initTree(self):
		bd = Bbdd()
		data = bd.select("factura_cliente", "n_orden", "cliente=" + str(self.id))
	...
	def editItem(self):
		self.mainWindows.editarFacturaC(self.id, self.itemSelected)

	def nuevaFactura(self):
		self.mainWindows.nuevaFacturaC(self.id)